...
class Bag extends Component {
    constructor(props) {
        ...
        this.renderProductSizesSwitcher = this.renderProductSizesSwitcher.bind(this);
            ...
            <div className="size_section">
                {product.attributes[0]?.items[0].value.includes('#') ?
                    product.attributes[0]?.items.map(size => <button
                        onClick={() => this.selectedSize(product, size, key)} key={size.id}
                        style={{backgroundColor: size.value}}
                        className={"color_button " + (product.selectedSize.value === size.value ?
                            'active_color_size' : '')}/>) :
                    product.attributes[0]?.items.map(size => <button
                        onClick={() => this.selectedSize(product, size, key)} key={size.id}
                        className={"size_button " + (product.selectedSize.value === size.value ?
                            'active_button_size' : '')}>{size.value}</button>)}
            </div>
        )
    }
            ...
            <div className="product_bag_image">
                <img onClick={() => {
                    this.prevProductImage(key)
                     ...
                     style={this.state.cardPageImageIndexes[key]?.currentIndex === 0 ? {display: "none"} : {}}
                     className="bag_image_left_arrow"
                     src={process.env.PUBLIC_URL + '/img/left_arrow.png'} alt=""/>
                <img onClick={() => {
                    this.nextProductImage(key)
                }}
                     style={this.state.cardPageImageIndexes[key]?.currentIndex === this.state.cardPageImageIndexes[key]?.imageCount - 1 ?
                         {display: "none"} : {}}
                     className="bag_image_right_arrow"
                     src={process.env.PUBLIC_URL + '/img/right_arrow.png'} alt=""/>
                <img className="bag_main_product_image"
                     src={product.gallery[this.getCurrentProductImage(key)]} alt=""/>
            </div>
        )
    }
